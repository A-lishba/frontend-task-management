...
//         );
//       } else {
//         await axios.post("http://localhost:3003/tasks/add", formData, getConfig());
//       }
...
//     try {
//       await axios.delete(`http://localhost:3003/tasks/delete/${id}`, getConfig());
//       fetchTasks();
//     } catch (err) {
...
//   const filteredTasks = onlyCompleted ? tasks.filter((t) => t.completed) : tasks;
...
//       {showForm && (
//         <form onSubmit={handleSubmit} className="max-w-md mx-auto bg-slate-800 p-6 rounded-xl mb-6">
...
//           <div key={task._id} className="border rounded px-2 shadow bg-gray-600 max-w-xs w-full">
//             <h2 className="text-xl font-bold text-center py-2">{task.title}</h2>
//             <p className="text-sm text-white">Due: {task.dueDate?.substring(0, 10)}</p>
//             <p className={`text-sm font-semibold ${task.completed ? "text-green-400" : "text-red-400"}`}>
...
export default function Dashboard() {
  ...
  // Find next upcoming task whenever tasks change
  useEffect(() => {
    if (tasks.length > 0) {
